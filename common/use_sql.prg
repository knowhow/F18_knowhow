...
FUNCTION use_sql_valute()
   ...
   cSql += "(CASE WHEN kurs1 IS NULL THEN 0::numeric(18,8) ELSE kurs1::numeric(18,8) END ),"
   cSql += "(CASE WHEN kurs2 IS NULL THEN 0::numeric(18,8) ELSE kurs2::numeric(18,8) END ),"
   cSql += "(CASE WHEN kurs3 IS NULL THEN 0::numeric(18,8) ELSE kurs3::numeric(18,8) END ),"
   cSql += "tip::char(1) "
...
FUNCTION use_sql_ks()
   ...
   cSql += "  datod, "
   cSql += "  datdo, "
   cSql += "  ( CASE WHEN strev IS NULL THEN 0::numeric(8,4) ELSE strev::numeric(8,4) END ), "
   cSql += "  ( CASE WHEN stkam IS NULL THEN 0::numeric(8,4) ELSE stkam::numeric(8,4) END ), "
   cSql += "  ( CASE WHEN den IS NULL THEN 0::numeric(15,6) ELSE den::numeric(15,6) END ), "
   cSql += "  tip::char(1), "
   cSql += "  ( CASE WHEN duz IS NULL THEN 0::numeric(4,0) ELSE duz::numeric(4,0) END ) "
   cSql += "FROM fmk.ks "
   cSQL += "ORDER BY id"
...
FUNCTION use_sql_tarifa( l_make_index )
   ...
   cSql += "  id, "
   cSql += "  naz, "
   cSql += "  ( CASE WHEN opp IS NULL THEN 0::numeric(6,2) ELSE opp::numeric(6,2) END ), "
   cSql += "  ( CASE WHEN ppp IS NULL THEN 0::numeric(6,2) ELSE ppp::numeric(6,2) END ), "
   cSql += "  ( CASE WHEN zpp IS NULL THEN 0::numeric(6,2) ELSE zpp::numeric(6,2) END ), "
   cSql += "  ( CASE WHEN vpp IS NULL THEN 0::numeric(6,2) ELSE vpp::numeric(6,2) END ), "
   cSql += "  ( CASE WHEN mpp IS NULL THEN 0::numeric(6,2) ELSE mpp::numeric(6,2) END ), "
   cSql += "  ( CASE WHEN dlruc IS NULL THEN 0::numeric(6,2) ELSE dlruc::numeric(6,2) END ), "
   cSql += "  match_code::char(10) "
