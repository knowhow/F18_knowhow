...
FUNCTION fakt_kartica()
   ...
   DO WHILE .T.
      @ form_x_koord() + 1, form_y_koord() + 2 SAY "Brza kartica (D/N)" GET cBrza PICT "@!" VALID cBrza $ "DN"
      READ
      IF gNW $ "DR"
         @ form_x_koord() + 2, form_y_koord() + 2 SAY "RJ (prazno svi) " GET cIdFirma VALID {|| Empty( cIdFirma ) .OR. cIdFirma == self_organizacija_id() .OR. P_RJ( @cIdFirma ), cIdFirma := Left( cIdFirma, 2 ), .T. }
      ELSE
      ...
      IF cBrza == "D"
         ...
         IF fID_J
            @ form_x_koord() + 3, form_y_koord() + 2 SAY "Roba " GET qqRoba PICT "@!" VALID {|| P_Roba( @qqRoba ), qqRoba := roba->id_j, .T. }
         ELSE
            @ form_x_koord() + 3, form_y_koord() + 2 SAY "Roba " GET qqRoba PICT "@!" VALID P_Roba( @qqRoba )
         ENDIF
      ELSE
         RPar( "c2", @qqRoba )
         qqRoba := PadR( qqRoba, 60 )
         @ form_x_koord() + 3, form_y_koord() + 2 SAY "Roba " GET qqRoba PICT "@!S40"
      ENDIF

      @ form_x_koord() + 4, form_y_koord() + 2 SAY "Od datuma "  GET dDatOd
      @ form_x_koord() + 4, Col() + 1 SAY "do"  GET dDatDo
      @ form_x_koord() + 5, form_y_koord() + 2 SAY "Prikaz rezervacija, reversa (D/N)   "  GET cRR   PICT "@!" VALID cRR $ "DN"
      @ form_x_koord() + 6, form_y_koord() + 2 SAY "Prethodno stanje (1-BEZ, 2-SA)      "  GET cPredh PICT"9" VALID cPredh $ "12"
      IF gVarC $ "12"
         @ form_x_koord() + 7, form_y_koord() + 2 SAY "Stanje prikazati sa Cijenom 1/2 (1/2) "  GET cTipVpc PICT "@!" VALID cTipVPC $ "12"
      ENDIF

      @ form_x_koord() + 8, form_y_koord() + 2 SAY "Naziv partnera (prazno - svi)"  GET qqPartn   PICT "@!"
      IF fakt->( FieldPos( "K1" ) ) <> 0 .AND. gDK1 == "D"
         @ form_x_koord() + 9, form_y_koord() + 2 SAY "K1" GET  cK1 PICT "@!"
         @ form_x_koord() + 10, form_y_koord() + 2 SAY "K2" GET  cK2 PICT "@!"
      ENDIF

      @ form_x_koord() + 12, form_y_koord() + 2 SAY "Prikaz kretanja cijena D/N"  GET cPPC PICT "@!" VALID cPPC $ "DN"
      @ form_x_koord() + 13, form_y_koord() + 2 SAY8 "Prikaži partnera za svaku stavku"  GET cPPartn PICT "@!" VALID cPPartn $ "DN"

      IF cBrza == "N"
         @ form_x_koord() + 15, form_y_koord() + 2 SAY "Svaka kartica na novu stranicu? (D/N)"  GET cOstran VALID cOstran $ "DN" PICT "@!"
      ELSE
         cOstran := "N"
      ENDIF

      IF _params[ "fakt_objekti" ]
         @ form_x_koord() + 16, form_y_koord() + 2 SAY "Uslov po objektima (prazno-svi)" GET _objekat_id VALID Empty( _objekat_id ) .OR. P_fakt_objekti( @_objekat_id )
      ENDIF
      ...
      IF cSintetika == "D" .AND.  IF( cBrza == "D", ROBA->tip == "S", .T. )
         @ form_x_koord() + 17, form_y_koord() + 2 SAY8 "Sintetički prikaz? (D/N) " GET  cSintetika PICT "@!" VALID cSintetika $ "DN"
      ELSE
         cSintetika := "N"
      ENDIF
      READ
