...
if gVarF=="3"
 if IzFMKINI("FAKT","KupacDesno","N",KUMPATH)=="D"
   @ prow(),6 SAY padr(get_mjesto_po_rj(cIdFirma)+", "+dtoc(datdok)+" godine",36)
   ?
   ?
   ...
   @ prow(),36 SAY padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(datdok)+" godine",36)
   ?
   ?
 ...
 if IzFMKINI("FAKT","KupacDesno","N",KUMPATH)=="D"
   ?? space(4),padr(get_mjesto_po_rj(cIdFirma)+", "+SrediDat(datdok),39) ; ?? gPB_ON+padc(alltrim(cTxt3a),30)+gPB_OFF
   ...
   ?? space(4),gPB_ON+padc(alltrim(cTxt3a),30)+gPB_OFF; ?? padl(get_mjesto_po_rj(cIdFirma)+", "+SrediDat(datdok),39)
...
? padr(get_mjesto_po_rj(cIdFirma)+", "+dtoc(datdok)+" godine",42)
