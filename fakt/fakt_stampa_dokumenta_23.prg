...
do while idfirma==cidfirma .and. idtipdok==cidtipdok .and. brdok==cbrdok .and. !eof()
	...
	if alltrim(podbr)=="."
		...
		if fDelphiRB
      			...
      			replace naziv with KonvZnWin(@cTxt1,gKonvZnWin), kolicina with transform(fakt_pripr->(kolicina()),pickol)
      			select fakt_pripr
    		else
			? space(gnLMarg)
      			if cIdTipdok=="12"
			...
			if fDelphiRB
          			select pom
          			replace  kolicina with fakt_pripr->(transform(iif(kolicina==0,0,nUk2/kolicina()) , piccdem)) ,;
                   		rabat with fakt_pripr->(cRab+"%"), dest with fakt_pripr->dest
     		...
     		if idtipdok $ "11#15#27"  // maloprodaja ili izlaz iz MP putem VP ili predr.MP
       			...
       			IF IzFMKINI("POREZI","PPUgostKaoPPU","D")=="D"
         			nMPVBP:=round( fakt_pripr->(cijena*nKurs*PrerCij()*kolicina())/(1+tarifa->zpp/100+tarifa->ppp/100)/(1+tarifa->opp/100) , nZaokr)
       			ELSE
         			nMPVBP:=round( fakt_pripr->(cijena*nKurs*PrerCij()*kolicina())/((1+tarifa->opp/100)*(1+tarifa->ppp/100)+tarifa->zpp/100) , nZaokr)
       			ENDIF
       			if tarifa->opp<>0
         			select por
		...
		if roba->tip="U"
     			...
     			aTxtR:=SjeciStr(trim(roba->naz)+Katbr(),40-IF(gNW=="R".and.idtipdok$"11#27",6,0))
      			if fdelphiRB
       				...
       				select pom
       				append blank // prvo se stavlja naziv!!
       				replace naziv with fakt_pripr->(trim(LEFT(roba->naz,40))+iif(!empty(ck1+ck2)," "+ck1+" "+ck2,"")+Katbr()+IspisiPoNar())
		...
		if fDelphiRB
			...
			if !cidtipdok$"11#27"
             			...
             			if gVarF=="2"
               				if fDelphiRB
                 				select pom
                 				replace CijenaMR with fakt_pripr->(transform(cijena*(1-rabat/100)*Koef(cDinDem),piccdem))
                 				select fakt_pripr
               				else
				...
				if fDelphiRB
                			select pom
                			replace POR with fakt_pripr->(cPor+"%")
                			select fakt_pripr
             			else
               				if !(gVarF=="3" .and. cidtipdok=="12")
                 				@ prow(),pcol()+1 SAY cPor+"%"
...
*{
local cMjesto:=padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok),iif(gFPZag=99,gnTMarg3,0)+39)
//local cMjesto:=padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok)+" godine",iif(gFPZag=99,gnTMarg3,0)+39)
...
if fdelphiRb
   ...
   NEXT
   if gDest
     if !EMPTY(fakt_pripr->DEST)
       cDest:=fakt_pripr->DEST
     else
       cDest:='--'
     endif
   ...
   if IzFmkIni("FAKT","Opcine","N",SIFPATH)=="D"
	...
	IF IzFMKINI("FAKT","KupacDesno","N",KUMPATH)=="D"
    		if !(cidtipdok$"10#06#16")
      			@ prow(),6 SAY padr(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok)+" godine",36)
      			?
    		endif
  	...
  	ELSE
		if !(cidtipdok$"10#06#16") .and. (!lPharmaMAC .or. !(cidtipdok$"20#19"))
      			//@ prow(),36 SAY padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok)+" godine",36)
			@ prow(),36 SAY padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok)+" godine",36)
    		endif
...
elseif (lTrebovanje .and. !Empty(fakt_pripr->idRNal))
  cStr:="IZLAZ MATERIJALA br. " + cStr		
  cStrRN:=TRIM("PO RADNOM NALOGU br. "+fakt_pripr->idRNal)
endif
...
if cidtipdok=="10"
 ...
 if !empty(_brnar)
   if fDelphiRB
     UzmiIzIni(cIniName,'Varijable','BrNar',_brnar,'WRITE')
      ...
      if gDest
       ?? "Ugovor/narudzba :",_brnar+", "+fakt_pripr->DEST
      else
 ...
 if fDelphiRB
   ...
   if gDest
     ?? space(18-LEN(fakt_pripr->DEST)), "Datum placanja:",_DatPl
     ?
   else
...
elseif cIdTipDok=="06"
 if fDelphiRB
   ...
   ?? SPACE(1)
   ?? padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok)+". godine",36)
...
elseif cIdTipDok=="16" .or. lPharmaMAC.and.cIdTipDok$"19#20"
 P_COND; ? space(gnLMarg); P_10CPI
 @ prow(), 43 SAY padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(ddatdok)+". godine",36)
endif
