...
// ---------------- MS 07.04.01
// ?? space(5);gPB_ON();?? padc(alltrim(cTxt3a),30);gPB_OFF(); ?? padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(datdok)+" godine",39)

aPom:=Sjecistr(cTxt3a,30)
IF IzFMKINI("FAKT","KupacDesno","N",KUMPATH)=="D"
  ?? space(5); ?? padr(get_mjesto_po_rj(cIdFirma)+", "+dtoc(datdok)+" godine",39); gPB_ON(); ?? padc(alltrim(aPom[1]),30); gPB_OFF()
  for i:=2 to len(aPom)
...
ELSE
  ?? space(5);gPB_ON();?? padc(alltrim(aPom[1]),30);gPB_OFF(); ?? padl(get_mjesto_po_rj(cIdFirma)+", "+dtoc(datdok)+" godine",39)
  for i:=2 to len(aPom)
