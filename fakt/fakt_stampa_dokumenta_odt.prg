...
STATIC FUNCTION _grupno_params( params )
   ...
   @ form_x_koord() + _x, form_y_koord() + 2 SAY "*** Stampa ODT dokumenata po zadanom uslovu:"

   ++ _x
   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Radna jedinica / vrsta:" GET _id_firma VALID !Empty( _id_firma )
   @ form_x_koord() + _x, Col() + 1 SAY "-" GET _id_tip_dok VALID !Empty( _id_tip_dok )

   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Za datum od:" GET _datum_od
   @ form_x_koord() + _x, Col() + 1 SAY "do:" GET _datum_do

   ++ _x
   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Brojevi dokumenata:" GET _brojevi PICT "@S45"

   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Obuhvati artikle:" GET cIdRoba PICT "@S45"

   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Obuhvati partnere:" GET _partneri PICT "@S45"

   ++ _x
   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Generisati grupno/jedan po jedan (1/2) ?" GET _tip_gen VALID _tip_gen $ "12"

   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Formirati PDF dokument (D/N) ?" GET _gen_pdf VALID _gen_pdf $ "DN"

   ++ _x
   ++ _x

   @ form_x_koord() + _x, form_y_koord() + 2 SAY "Prebaci na lokaciju:" GET _na_lokaciju PICT "@S40"
