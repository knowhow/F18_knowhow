...
FUNCTION fin_gen_psuban_stampa_nalozi( lAuto, dDatNal )
   ...
   IF lAuto
      // _print_opt := "D"
      Box(, 3, 75 )
      @ form_x_koord() + 0, form_y_koord() + 2 SAY8 "Formiranje sintetičkih i analitičkih stavki"
   ENDIF
   ...
   DO WHILE !Eof()
      ...
      IF AScan( aNalozi, cIdFirma + cIdVN + cBrNal ) == 0
         ...
         IF lAuto
            @ form_x_koord() + 2, form_y_koord() + 2 SAY "Formirana sintetika i analitika za nalog:" + cIdFirma + "-" + cIdVN + "-" + cBrNal
         ENDIF
      ENDIF
...
FUNCTION fin_gen_sint_stavke( lAuto, dDatNal )
   ...
   DO WHILE !Eof()
      ...
      IF !lAuto
         Box(, 2, 58 )
         @ form_x_koord() + 1, form_y_koord() + 2 SAY8 "Štampanje analitike/sintetike za nalog " + cIdfirma + "-" + cIdvn + "-" + cBrnal + " ?"  GET cDN PICT "@!" VALID cDN $ "DN"
         READ
         BoxC()
      ENDIF
