...
FUNCTION fin_automatsko_zatvaranje_otvorenih_stavki( lAuto, cKto, cPtn )
   ...
   IF !lAuto
      ...
      @ form_x_koord() + 1, form_y_koord() + 2 SAY "AUTOMATSKO ZATVARANJE STAVKI"
   ...
   @ form_x_koord() + 1, form_y_koord() + 2 SAY "Zatvoreno:"
   @ form_x_koord() + 1, form_y_koord() + 12 SAY nCntZatvoreno
   ...
   DO WHILE !Eof() .AND. idfirma == cIdfirma .AND. cIdKonto == IdKonto
      ...
      IF Abs( Round( nDugBHD - nPotBHD, 3 ) ) <= gnLOSt .AND. cOtvSt == "1"
         ...
         GO nRbrStartBrDok
         @ form_x_koord() + 1, form_y_koord() + 2 SAY cIdKonto + "-" + cIdPartner + "/" + cBrDok
         @ form_x_koord() + 1, Col() + 2 SAY ++nCntZatvoreno PICT '99999'
...
STATIC FUNCTION ponisti_markere_postojecih_stavki( cIdFirma, cIdKonto, cIdPartner )
   ...
   @ form_x_koord() + 1, form_y_koord() + 2 SAY8 "Brišem markere postojećih stavki tabele..."
   ...
   DO WHILE !Eof() .AND. idfirma == cidfirma .AND. cIdKonto = IdKonto
      ...
      @ form_x_koord() + 2, form_y_koord() + 2 SAY "nalog: " + _rec[ "idvn" ] + "-" + AllTrim( _rec[ "brnal" ] ) + " / stavka: " + Str( _rec[ "rbr" ], 5, 0 )
