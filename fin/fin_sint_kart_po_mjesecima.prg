...
FUNCTION fin_sint_kart_po_mjesecima()
   ...
   DO WHILE .T.
      SET CURSOR ON
      @ form_x_koord() + 1, form_y_koord() + 2 SAY8 "KARTICA (SINTETIČKI KONTO) PO MJESECIMA"
      ...
      ENDIF
      @ form_x_koord() + 3, form_y_koord() + 2 SAY "Konto: " GET qqKonto PICTURE "@S50"
      @ form_x_koord() + 4, form_y_koord() + 2 SAY "Datum od:" GET dDatOd
      @ form_x_koord() + 4, Col() + 2 SAY "do:" GET dDatDo
      cIdRJ := ""
      IF gFinRj == "D" .AND. gSAKrIz == "D"
         cIdRJ := "999999"
         @ form_x_koord() + 5, form_y_koord() + 2 SAY "Radna jedinica (999999-sve): " GET cIdRj
      ENDIF
      READ;  ESC_BCR
