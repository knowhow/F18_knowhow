...
/* fin_spec_otv_stavke_po_dospijecu(lKartica)
 *  Otvorene stavke grupisano po brojevima veze
 */

FUNCTION fin_spec_otv_stavke_po_dospijecu( lKartica )
   ...
   DO WHILE !Eof() .AND. idfirma == cIdfirma .AND. cIdKonto == IdKonto
      ...
      IF PRow() > 58 + dodatni_redovi_po_stranici()
         FF
         Zfin_spec_otv_stavke_po_dospijecu( nil, nil, PICPIC )
      ENDIF
   ...
   DO WHILE !Eof()
      ...
      IF PRow() > 52 + dodatni_redovi_po_stranici()
         FF
         Zfin_spec_otv_stavke_po_dospijecu( .T., nil, PICPIC )
         fPrviProlaz := .F.
      ENDIF
   ...
   IF cPoRn == "D" .AND. Len( cSvi ) < Len( idpartner ) .AND. ;
      ...
      // prikazimo total
      FF
      Zfin_spec_otv_stavke_po_dospijecu( .T., .T., PICPIC )
      ? m2 := StrTran( M, "-", "=" )
      IF cSaRokom == "D"
         FOR i := 1 TO Len( anInterUV )
