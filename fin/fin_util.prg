...
STATIC FUNCTION _g_gr_date()
   ...
   Box(, 1, 45 )
   @ form_x_koord() + 1, form_y_koord() + 2 SAY "Unesi granicni datum" GET dDate
   READ
   BoxC()
...
FUNCTION BBMnoziSaK( cTip )
   ...
   IF cTip == ValDomaca() .AND. my_get_from_ini( "FIN", "BrutoBilansUDrugojValuti", "N", KUMPATH ) == "D"
      Box(, 5, 70 )
      @ form_x_koord() + 2, form_y_koord() + 2 SAY "Pomocna valuta      " GET cBBV PICT "@!" VALID ImaUSifVal( cBBV )
      @ form_x_koord() + 3, form_y_koord() + 2 SAY "Omjer pomocna/domaca" GET nBBK WHEN {|| nBBK := OmjerVal2( cBBV, cTip ), .T. } PICT "999999999.999999999"
      READ
      BoxC()
   ELSE
      cBBV := cTip
