...
FUNCTION kalk_get_1_14()
   ...
   ELSE
      @ form_x_koord() + 6, form_y_koord() + 2   SAY8 "KUPAC: "; ?? _IdPartner
      @ form_x_koord() + 7, form_y_koord() + 2   SAY8 "Faktura Broj: "; ?? _BrFaktP
      @ form_x_koord() + 7, Col() + 2 SAY8 "Datum: "; ?? _DatFaktP

      _IdZaduz := ""
      _Idkonto := "2110"
      @ form_x_koord() + 9, form_y_koord() + 2 SAY8 "Magacinski konto razdužuje "; ?? _IdKonto2
      // IF gNW <> "X"
      // @ form_x_koord() + 9, form_y_koord() + 40 SAY8 "Razdužuje: "; ?? _IdZaduz2
      // ENDIF
   ENDIF
   ...
   @ form_x_koord() + 13, form_y_koord() + 2    SAY8 "Nab.Cjena "  GET _NC  PICTURE PicDEM   VALID kalk_valid_kolicina_mag()
   ...
   @ form_x_koord() + 16, form_y_koord() + 2 SAY8 "PDV (%)  " + Transform( _MPC, "99.99" )
...
FUNCTION pPDV14( lRet )
   ...
   DevPos( form_x_koord(), form_y_koord() + 41 )
   IF roba->tip $ "VKX"
      // nista ppp
   ELSE
