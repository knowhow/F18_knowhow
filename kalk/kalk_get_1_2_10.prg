...
FUNCTION kalk_get_1_10()
   ...
   IF nRbr == 1  .OR. !kalk_is_novi_dokument() .OR. gMagacin == "1"
      ...
      @ form_x_koord() + nX, form_y_koord() + 2 SAY8 "DOBAVLJAČ:" GET _IdPartner PICT "@!" ;
      ...
      @ form_x_koord() + nX, 50 SAY "Broj fakture:" GET _BrFaktP VALID !Empty ( _brFaktP )
      @ form_x_koord() + nX, Col() + 1 SAY "Datum:" GET _DatFaktP VALID {||  datum_not_empty_upozori_godina( _datFaktP, "Datum fakture" ) }


      ++ nX
      _kord_x := form_x_koord() + nX

      @ form_x_koord() + nX, form_y_koord() + 2 SAY "Magacinski Konto zaduzuje" GET _IdKonto valid {|| P_Konto( @_IdKonto ), ispisi_naziv_sifre( F_KONTO, _idkonto, _kord_x, 40, 30 ) } PICT "@!"

      // IF gNW <> "X"
      // @ form_x_koord() + nX, form_y_koord() + 42  SAY "Zaduzuje: " GET _IdZaduz  PICT "@!" VALID Empty( _idZaduz ) .OR. p_partner( @_IdZaduz )
      // ENDIF

      IF !Empty( cRNT1 )
         @ form_x_koord() + nX, form_y_koord() + 60  SAY "Rad.nalog:" GET _IdZaduz2  PICT "@!"
      ENDIF
   ...
   ELSE
      ...
      @ form_x_koord() + nX, form_y_koord() + 2 SAY "DOBAVLJAC: "
      ?? _IdPartner
      @ form_x_koord() + nX, Col() + 1 SAY "Faktura dobavljaca - Broj: "
      ?? _BrFaktP
      @ form_x_koord() + nX, Col() + 1 SAY "Datum: "
      ?? _DatFaktP

      ++ nX
      @ form_x_koord() + nX, form_y_koord() + 2 SAY "Magacinski Konto zaduzuje "
      ?? _IdKonto

      // IF gNW <> "X"
      // @ form_x_koord() + nX, form_y_koord() + 42 SAY "Zaduzuje: "
      // ?? _IdZaduz
      // ENDIF
      ino_dobavljac_set_konverzija_valute( _idpartner, @s_cKonverzijaValuteDN )
   ...
   ++ nX
   @ form_x_koord() + nX, form_y_koord() + 2 SAY "Fakturna cijena:"

   // IF gDokKVal == "D"
   @ form_x_koord() + nX, Col() + 1 SAY "EUR (D/N)->" GET s_cKonverzijaValuteDN VALID kalk_ulaz_preracun_fakturne_cijene( s_cKonverzijaValuteDN ) PICT "@!"
   // ENDIF

   @ form_x_koord() + nX, form_y_koord() + _unos_left GET _fcj PICT gPicNC VALID {|| _fcj > 0 .AND. _set_konv( @_fcj, @s_cKonverzijaValuteDN ) } WHEN V_kol10()

   ++ nX
   @ form_x_koord() + nX, form_y_koord() + 2 SAY "Rabat (%):"
   @ form_x_koord() + nX, form_y_koord() + _unos_left GET _Rabat PICT PicDEM

   /*
   IF gNW <> "X" .OR. gVodiKalo == "D"
      ++ nX
      @ form_x_koord() + nX, form_y_koord() + 2 SAY "Normalni . kalo:"
      @ form_x_koord() + nX, form_y_koord() + _unos_left GET _GKolicina PICTURE PicKol
      ++ nX
      @ form_x_koord() + nX, form_y_koord() + 2 SAY "Preko  kalo:    "
      @ form_x_koord() + nX, form_y_koord() + _unos_left GET _GKolicin2 PICTURE PicKol
   ENDIF
   */
...
STATIC FUNCTION kalk_get_2_10( x_kord, cIdPartner )
   ...
   IF lSaTroskovima
      ...
      // TROSKOVNIK
      @ form_x_koord() + nX, form_y_koord() + 2 SAY "Raspored troskova kalkulacije ->"

      @ form_x_koord() + nX, form_y_koord() + _unos_left + 10 SAY c10T1 + cSPom GET _TPrevoz VALID _TPrevoz $ "%AURT" PICTURE "@!"
      @ form_x_koord() + nX, Col() + 2 GET _Prevoz PICT  PicDEM

      ++ nX
      @ form_x_koord() + nX, form_y_koord() + _unos_left + 10 SAY c10T2 + cSPom  GET _TBankTr VALID _TBankTr $ "%AURT" PICT "@!"
      @ form_x_koord() + nX, Col() + 2 GET _BankTr PICT PicDEM

      ++ nX
      @ form_x_koord() + nX, form_y_koord() + _unos_left + 10 SAY c10T3 + cSPom GET _TSpedTr VALID _TSpedTr $ "%AURT" PICT "@!"
      @ form_x_koord() + nX, Col() + 2 GET _SpedTr PICT PicDEM

      ++ nX
      @ form_x_koord() + nX, form_y_koord() + _unos_left + 10 SAY c10T4 + cSPom GET _TCarDaz VALID _TCarDaz $ "%AURT" PICTURE "@!"
      @ form_x_koord() + nX, Col() + 2 GET _CarDaz PICT PicDEM

      ++ nX
      @ form_x_koord() + nX, form_y_koord() + _unos_left + 10 SAY c10T5 + cSPom GET _TZavTr VALID _TZavTr $ "%AURT" PICTURE "@!"
      @ form_x_koord() + nX, Col() + 2 GET _ZavTr PICT PicDEM
   ...
   @ form_x_koord() + nX, form_y_koord() + _unos_left GET _nc PICT gPicNC WHEN {|| kalk_nabcj(), .T. }
   ...
   ++ nX
   @ form_x_koord() + nX, form_y_koord() + 2    SAY8 "Magacin. Marža           :" GET _TMarza VALID _Tmarza $ "%AU" PICTURE "@!"
   @ form_x_koord() + nX, Col() + 2 GET _Marza PICT PicDEM
   @ form_x_koord() + nX, Col() + 1 GET fMarza PICT "@!" VALID {|| Marza( fMarza ), fMarza := " ", .T. }

   // PRODAJNA CIJENA / PLANSKA CIJENA
   ++ nX
   IF koncij->naz == "P2"
      @ form_x_koord() + nX, form_y_koord() + 2    SAY "PLANSKA CIJENA  (PLC)       :"
   ELSE
      @ form_x_koord() + nX, form_y_koord() + 2    SAY "PROD.CJENA BEZ PDV   :"
   ENDIF

   @ form_x_koord() + nX, form_y_koord() + _unos_left GET _vpc PICT PicDEM VALID {|| MarzaVP( _Idvd, ( fMarza == "F" ) ), .T. }
   ...
   IF ( gMpcPomoc == "D" )   // VPC se izracunava pomocu MPC cijene
      ...
      @ form_x_koord() + nX, form_y_koord() + 2 SAY "PROD.CJENA SA PDV:"
