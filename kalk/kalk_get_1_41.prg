...
FUNCTION kalk_get_1_41()
   ...
   IF _idvd == "41"
      ...
      @  form_x_koord() + 6,  form_y_koord() + 2 SAY "KUPAC:" GET _IdPartner PICT "@!" VALID Empty( _IdPartner ) .OR. p_partner( @_IdPartner, 5, 30 )
      @  form_x_koord() + 7,  form_y_koord() + 2 SAY "Faktura Broj:" GET _BrFaktP
      @  form_x_koord() + 7, Col() + 2 SAY "Datum:" GET _DatFaktP

   ELSEIF _idvd == "43"

      @  form_x_koord() + 6,  form_y_koord() + 2 SAY "DOBAVLJAC KOMIS.ROBE:" GET _IdPartner PICT "@!" VALID Empty( _IdPartner ) .OR. p_partner( @_IdPartner, 5, 30 )
   ...
   // IF gNW <> "X"
   // @ form_x_koord() + 8, form_y_koord() + 50  SAY "Razduzuje: "   GET _IdZaduz  PICT "@!" VALID Empty( _idZaduz ) .OR. p_partner( @_IdZaduz, 21, 5 )
   // ENDIF
...
*/
   ...
   IF ( ( _idvd <> "47" ) .AND. roba->tip != "T" )
      ...
      @ form_x_koord() + 14, form_y_koord() + 2 SAY "NC  :" GET _fcj PICT picdem ;
         VALID {|| lRet := kalk_valid_kolicina_prod(), _tprevoz := "A", _prevoz := 0, _nc := _fcj, lRet }

      @ form_x_koord() + 15, form_y_koord() + 40 SAY "MP marza:" GET _TMarza2  VALID _Tmarza2 $ "%AU" PICTURE "@!"
      @ form_x_koord() + 15, Col() + 1 GET _Marza2 PICTURE  PicDEM

   ENDIF


   @ form_x_koord() + 17, form_y_koord() + 2 SAY "PRODAJNA CJENA  (PC):"


   @ form_x_koord() + 17, form_y_koord() + 50 GET _mpc PICT PicDEM WHEN W_MPC_( IdVd, .F., @aPorezi ) VALID V_Mpc_( _IdVd, .F., @aPorezi )
   ...
   @ form_x_koord() + 20, form_y_koord() + 2 SAY "MPC SA PDV    :"

   @ form_x_koord() + 20, form_y_koord() + 50 GET _mpcsapp PICT PicDEM VALID V_MpcSaPP_( _IdVd, .F., @aPorezi, .T. )
