...
FUNCTION kalk_get_1_95()
   ...
   IF nRbr == 1 .OR. !kalk_is_novi_dokument() .OR. gMagacin == "1"
      ...
      @  form_x_koord() + 5, form_y_koord() + 2   SAY "Dokument Broj:" GET _BrFaktP VALID !Empty( _BrFaktP )
      @  form_x_koord() + 5, Col() + 1 SAY "Datum:" GET _DatFaktP  VALID { ||  datum_not_empty_upozori_godina( _datFaktP, "Datum fakture" ) }


      _IdZaduz := ""
      @ form_x_koord() + 8, form_y_koord() + 2 SAY8 "Magacinski konto razdužuje"  GET _IdKonto2 ;
         VALID Empty( _IdKonto2 ) .OR. P_Konto( @_IdKonto2, 21, 5 )
      // IF gNW <> "X"
      // @ form_x_koord() + 8, form_y_koord() + 40 SAY "Razdužuje:" GET _IdZaduz2   PICT "@!"  VALID Empty( _idZaduz2 ) .OR. p_partner( @_IdZaduz2, 21, 5 )
      // ELSE
      IF !Empty( cRNT1 ) .AND. _idvd $ "97#96#95"
         IF ( IsRamaGlas() )
            @ form_x_koord() + 8, form_y_koord() + 60 SAY "Rad.nalog:" GET _IdZaduz2 PICT "@!" VALID RadNalOK()
         ELSE
            @ form_x_koord() + 8, form_y_koord() + 60 SAY "Rad.nalog:" GET _IdZaduz2   PICT "@!"
         ENDIF
      ENDIF
      // ENDIF
      IF _idvd $ "97#96#95"    // ako je otprema, gdje to ide

         @ form_x_koord() + 9, form_y_koord() + 2   SAY "Konto zaduzuje            " GET _IdKonto VALID  Empty( _IdKonto ) .OR. P_Konto( @_IdKonto, 21, 5 ) PICT "@!"

         IF ( _idvd == "95" .AND. IsVindija() )

            @ form_x_koord() + 9, form_y_koord() + 40 SAY "Šifra veze otpisa:" GET _IdPartner  VALID Empty( _idPartner ) .OR. p_partner( @_IdPartner, 21, 5 ) PICT "@!"

         ELSEIF gMagacin == "1"
            @ form_x_koord() + 9, form_y_koord() + 40 SAY8 "Partner zadužuje:" GET _IdPartner  VALID Empty( _idPartner ) .OR. p_partner( @_IdPartner, 21, 5 ) PICT "@!"
   ...
   ELSE
      ...
      @  form_x_koord() + 6, form_y_koord() + 2   SAY "Dokument Broj: "; ?? _BrFaktP
      @  form_x_koord() + 6, Col() + 2 SAY "Datum: "; ?? _DatFaktP
      _IdZaduz := ""
      @ form_x_koord() + 8, form_y_koord() + 2 SAY "Magacinski konto razduzuje "; ?? _IdKonto2
      @ form_x_koord() + 9, form_y_koord() + 2 SAY "Konto zaduzuje "; ?? _IdKonto
      // IF gNW <> "X"
      // @ form_x_koord() + 9, form_y_koord() + 40 SAY "Razduzuje: "; ?? _IdZaduz2
      // ENDIF
   ENDIF
   ...
   IF _TBankTr <> "X"
      ...
      @ form_x_koord() + 13, form_y_koord() + 30   SAY "Srednja nc "; @ form_x_koord() + 13, Col() + 2 SAY nc2 PICT pickol
   ...
   SELECT kalk_pripr
   @ form_x_koord() + 14, form_y_koord() + 2  SAY "NAB.CJ   "  GET _NC  PICTURE gPicNC  VALID kalk_valid_kolicina_mag()
   PRIVATE _vpcsappp := 0
