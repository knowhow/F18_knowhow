...
FUNCTION kalk_finansijski_obrt()
   ...
   cKartica := "D"
   DO WHILE .T.
      @ form_x_koord() + 1, form_y_koord() + 2 SAY "Konta prodavnice:" GET qqKonto PICT "@!S50"
      @ form_x_koord() + 3, form_y_koord() + 2 SAY "tekuci promet je period:" GET dDatOd
      @ form_x_koord() + 3, Col() + 2 SAY "do" GET dDatDo
      @ form_x_koord() + 4, form_y_koord() + 2 SAY "Kriterij za robu :" GET qqRoba PICT "@!S50"
      @ form_x_koord() + 6, form_y_koord() + 2 SAY "Magacin u koji se vrsi povrat rekl. robe:" GET cIdKPovrata PICT "@!"
      @ form_x_koord() + 8, form_y_koord() + 2 SAY "Prikaz kolicina:" GET cKolDN PICT "@!" VALID cKolDN $ "DN"
      @ form_x_koord() + 9, form_y_koord() + 2 SAY "Cijena (P-prodajna,N-nabavna):" GET cCijena PICT "@!" VALID cCijena $ "PN"
      @ form_x_koord() + 10, form_y_koord() + 2 SAY "VP sa uracunatim rabatom (D/N)?" GET cVpRab PICT "@!" VALID cVpRab $ "DN"
      @ form_x_koord() + 11, form_y_koord() + 2 SAY "Prodaja pocinje od 'Datum od' (D/N)?" GET cPrDatOd PICT "@!" VALID cPrDatOd $ "DN"
      READ
      nKorekcija := 12 / ( Month( dDatDo ) -Month( dDatOd ) + 1 )
      @ form_x_koord() + 12, form_y_koord() + 2 SAY "Korekcija (12/broj radnih mjeseci):" GET nKorekcija PICT "999.99"
      @ form_x_koord() + 13, form_y_koord() + 2 SAY "Ostampati legendu za kolone " GET cLegenda PICT "@!" VALID cLegenda $ "DN"
      @ form_x_koord() + 14, form_y_koord() + 2 SAY "Uslov po pl.vrsta " GET cPlVrsta PICT "@!"
      @ form_x_koord() + 15, form_y_koord() + 2 SAY "Izdvoji grupe: " GET cGrupeK1
      @ form_x_koord() + 16, form_y_koord() + 2 SAY "(npr. 0001;0006;0019;)"
      @ form_x_koord() + 17, form_y_koord() + 2 SAY "Uslov po K9 " GET cK9 PICT "@!"
      READ
      ESC_BCR
