...
STATIC FUNCTION get_vars( vars )
   ...
   Box(, 1, 40 )
   @ form_x_koord() + 1, form_y_koord() + 2 SAY "Broj dokumenta:"
   @ form_x_koord() + 1, Col() + 1 GET _firma
   @ form_x_koord() + 1, Col() + 1 SAY "-" GET _tip VALID !Empty( _tip )
   @ form_x_koord() + 1, Col() + 1 SAY "-" GET _broj VALID !Empty( _broj )
   READ
   BoxC()
