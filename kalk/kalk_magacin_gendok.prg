...
FUNCTION Iz12u97()
   ...
   Box(, 9, 75 )
   @ form_x_koord() + 0, form_y_koord() + 5 SAY "FORMIRANJE DOKUMENTA 96/97 NA OSNOVU DOKUMENTA 11/12"
   ...
   @ form_x_koord() + 4, form_y_koord() + 2 SAY "Dokument koji se formira (96/97)" GET cIdVdI VALID cIdVdI $ "96#97"
   @ form_x_koord() + 5, form_y_koord() + 2 SAY "Datum dokumenta koji se formira" GET dDatDok VALID !Empty( dDatDok )
   @ form_x_koord() + 7, form_y_koord() + 2 SAY "Prenijeti na konto (prazno-ne prenositi)" GET cKontoSklad
   READ
   ESC_BCR
   BoxC()
...
FUNCTION MNivPoProc()
   ...
   @ form_x_koord() + 4, m_Y + 2 SAY "(1) promjena prema stopama iz polja ROBA->N1"
   @ form_x_koord() + 5, m_Y + 2 SAY "(2) promjena prema stopama iz polja ROBA->N2"
   @ form_x_koord() + 6, m_Y + 2 SAY "(3) promjena prema jedinstvenoj stopi      ?"  GET cVarijanta VALID cVarijanta $ "123"
   read; ESC_BCR
...
FUNCTION KorekPC()
   ...
   Box(, 6, 50 )
   @ form_x_koord() + 1, form_y_koord() + 2 SAY "Magacinski konto" GET cMagac PICT "@!" VALID P_konto( @cMagac )
   @ form_x_koord() + 2, form_y_koord() + 2 SAY "Sravniti do odredjenog datuma:" GET cSravnitiD VALID cSravnitiD $ "DN" PICT "@!"
   @ form_x_koord() + 4, form_y_koord() + 2 SAY "Uvijek nivelisati na VPC iz sifrarnika:" GET cUvijekSif VALID cUvijekSif $ "DN" PICT "@!"
   read;ESC_BCR
...
FUNCTION kalk_gen_16_iz_96()
   ...
   Box(, 6, 75 )
   @ form_x_koord() + 0, form_y_koord() + 5 SAY "FORMIRANJE DOKUMENTA 16 NA OSNOVU DOKUMENTA 96"
...
FUNCTION Iz16u14()
   ...
   Box(, 8, 75 )
   @ form_x_koord() + 0, form_y_koord() + 5 SAY "FORMIRANJE DOKUMENTA 14 NA OSNOVU DOKUMENTA 16"
   @ form_x_koord() + 2, form_y_koord() + 2 SAY "Dokument: " + cIdFirma + "-"
   @ Row(), Col() SAY cIdVdU
   @ Row(), Col() SAY "-" GET cBrDokU VALID is_kalk_postoji_dokument( cIdFirma, cIdVdU, cBrDokU )
   @ form_x_koord() + 3, form_y_koord() + 2 SAY "Datum dokumenta koji se formira" GET dDatDok VALID !Empty( dDatDok )
   @ form_x_koord() + 4, form_y_koord() + 2 SAY "Broj fakture" GET cBrFaktP
   @ form_x_koord() + 5, form_y_koord() + 2 SAY "Datum fakture" GET dDatFaktP
   @ form_x_koord() + 6, form_y_koord() + 2 SAY "Kupac" GET cIdPartner VALID p_partner( @cIdPartner )
   READ; ESC_BCR
   BoxC()
