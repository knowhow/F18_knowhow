...
FUNCTION korekcija_nabavne_cijene_sa_zadnjom_ulaznom( nKolicina, nZadnjiUlazKol, nZadnjaUlaznaNC, nSrednjaNabavnaCijena, lSilent )
   ...
   IF Abs( nOdst ) > prag_odstupanja_nc_sumnjiv()
      ...
      IF !lSilent
         ...
         @ form_x_koord() + nX, form_y_koord() + 2   SAY     "Artikal: " + AllTrim( _idroba ) + "-" + PadR( roba->naz, 20 )
         nX += 2
         @ form_x_koord() + nX++, form_y_koord() + 2 SAY8 "  količina na stanju: " + AllTrim( say_kolicina( nKolicina ) )
         @ form_x_koord() + nX, form_y_koord() + 2 SAY8 "            Srednja NC: " + AllTrim( say_cijena( nSrednjaNabavnaCijena ) ) + " <"
         nX  += 2
         @ form_x_koord() + nX++, form_y_koord() + 2 SAY8 "količina zadnji ulaz: " + AllTrim( say_kolicina( nZadnjiUlazKol ) )
         @ form_x_koord() + nX++, form_y_koord() + 2 SAY8 "      NC Zadnji ulaz: " + AllTrim( say_cijena( nZadnjaUlaznaNC ) ) + " <"
         nX += 2
         @ form_x_koord() + nX++, form_y_koord() + 2 SAY8 " Korigovati NC na zadnju ulaznu: D/N ?"  GET cDn VALID cDn $ "DN" PICT "@!"
...
FUNCTION V_RabatV()
   ...
   IF Round( nRVPC - _vpc, 4 ) <> 0  .AND. gMagacin == "2"
      IF nRVPC == 0
         Beep( 1 )
         Box(, 3, 60 )
         @ form_x_koord() + 1, m_Y + 2 SAY "Roba u sifrarniku ima " + cPom + " = 0 !??"
         @ form_x_koord() + 3, form_y_koord() + 2 SAY "Unesi " + cPom + " u sifrarnik:" GET _vpc PICT picdem
   ...
   @ form_x_koord() + 15, form_y_koord() + 41  SAY "PC b.pdv.-RAB:"


   IF roba->tip == "V"
      @ form_x_koord() + 15, Col() + 1 SAY _Vpc / ( 1 + _PORVT ) -_VPC * _RabatV / 100 PICT picdem
   ELSEIF roba->tip == "X"
      @ form_x_koord() + 15, Col() + 1 SAY _Vpc * ( 1 -_RabatV / 100 ) - _MPCSAPP / ( 1 + _PORVT ) * _PORVT PICT picdem
   ELSE
      @ form_x_koord() + 15, Col() + 1 SAY _Vpc / ( 1 + _PORVT ) * ( 1 -_RabatV / 100 ) PICT picdem
   ENDIF
