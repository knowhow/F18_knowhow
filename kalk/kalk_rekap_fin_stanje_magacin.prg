...
FUNCTION rekap_finansijsko_stanje_magacin()
   ...
   DO WHILE .T.
      ...
      @ form_x_koord() + 5, form_y_koord() + 2 SAY "Tarife  " GET qqTarifa PICT "@!S50"
      @ form_x_koord() + 6, form_y_koord() + 2 SAY "Artikli " GET qqRoba   PICT "@!S50"
      @ form_x_koord() + 7, form_y_koord() + 2 SAY "Vrste dokumenata  " GET qqIDVD PICT "@!S30"
      @ form_x_koord() + 8, form_y_koord() + 2 SAY "Datum od " GET dDatOd
      @ form_x_koord() + 8, Col() + 2 SAY "do" GET dDatDo
      @ form_x_koord() + 9, form_y_koord() + 2 SAY "Prikazati i ako je saldo=0 ? (D/N)" GET cNula  VALID cNula $ "DN" PICT "@!"
      read; ESC_BCR
