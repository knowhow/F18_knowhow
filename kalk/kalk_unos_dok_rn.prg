...
FUNCTION Get1_RN()
   ...
   IF nRbr == 1  .OR. !kalk_is_novi_dokument() .OR. gMagacin == "1"
      @  form_x_koord() + 6, form_y_koord() + 2   SAY8 "  ZATVORITI RADNI NALOG :" GET _IdZaduz2 PICT "@!"
      @  form_x_koord() + 7, form_y_koord() + 2   SAY8 "Mag. proizvodnje u toku :" GET _IdKonto2 PICT "@!" VALID P_Konto( @_IdKonto2 )
      @ form_x_koord() + 10, form_y_koord() + 2   SAY8 "Mag. gotovih proizvoda zadužuje:" GET _IdKonto VALID  P_Konto( @_IdKonto, 24 ) PICT "@!"
      READ
      _BrFaktP := _idzaduz2
      ...
      SELECT kalk_pripr
      // IF gNW <> "X"
      // @ form_x_koord() + 10, form_y_koord() + 42  SAY "Zaduzuje: "   GET _IdZaduz  PICT "@!" VALID Empty( _idZaduz ) .OR. p_partner( @_IdZaduz, 24 )
      // ENDIF
      READ
   ...
   ELSE
      ...
      @ form_x_koord() + 10, form_y_koord() + 2   SAY "Mag. gotovih proizvoda zaduzuje "
      ?? _IdKonto
      // IF gNW <> "X"
      // @ form_x_koord() + 10, form_y_koord() + 42  SAY "Zaduzuje: "; ?? _IdZaduz
      // ENDIF
   ENDIF


   @ form_x_koord() + 11, form_y_koord() + 66 SAY "Tarif.br->"
   @ form_x_koord() + 12, form_y_koord() + 2  SAY "Proizvod  " GET _IdRoba PICT "@!" ;
   ...
   @ form_x_koord() + 15, form_y_koord() + 2   SAY "N.CJ.(DEM/JM):"
   @ form_x_koord() + 15, form_y_koord() + 50  GET _FCJ PICTURE PicDEM VALID _fcj > 0 when {|| _fcj := iif( nRbr > 1 .AND. kalk_is_novi_dokument(), _vpc, _fcj ), V_kol10() }

/*
   --IF gNW <> "X"
      @ form_x_koord() + 18, form_y_koord() + 2   SAY "Transport. kalo:"
      @ form_x_koord() + 18, form_y_koord() + 40  GET _GKolicina PICTURE PicKol

      @ form_x_koord() + 19, form_y_koord() + 2   SAY "Ostalo kalo:    "
      @ form_x_koord() + 19, form_y_koord() + 40  GET _GKolicin2 PICTURE PicKol
   ENDIF
*/
...
FUNCTION Get2_RN()
   ...
   @ form_x_koord() + 2, form_y_koord() + 2     SAY cRNT1 + cSPom GET _TPrevoz VALID _TPrevoz $ "%AUR" PICTURE "@!"
   @ form_x_koord() + 2, form_y_koord() + 40    GET _Prevoz PICTURE  PicDEM

   @ form_x_koord() + 3, form_y_koord() + 2     SAY cRNT2 + cSPom  GET _TBankTr VALID _TBankTr $ "%AUR" PICT "@!"
   @ form_x_koord() + 3, form_y_koord() + 40    GET _BankTr PICTURE PicDEM

   @ form_x_koord() + 4, form_y_koord() + 2     SAY cRNT3 + cSPom GET _TSpedTr VALID _TSpedTr $ "%AUR" PICT "@!"
   @ form_x_koord() + 4, form_y_koord() + 40    GET _SpedTr PICTURE PicDEM

   @ form_x_koord() + 5, form_y_koord() + 2     SAY cRNT4 + cSPom GET _TCarDaz VALID _TCarDaz $ "%AUR" PICTURE "@!"
   @ form_x_koord() + 5, form_y_koord() + 40    GET _CarDaz PICTURE PicDEM

   @ form_x_koord() + 6, form_y_koord() + 2     SAY cRNT5 + cSPom GET _TZavTr VALID _TZavTr $ "%AUR" PICTURE "@!"
   @ form_x_koord() + 6, form_y_koord() + 40    GET _ZavTr PICTURE PicDEM ;
      VALID {|| kalk_nabcj(), .T. }

   @ form_x_koord() + 8, form_y_koord() + 2     SAY "CIJENA KOST.  "
   @ form_x_koord() + 8, form_y_koord() + 50    GET _NC     PICTURE PicDEM

   IF koncij->naz <> "N1"  // vodi se po vpc
      PRIVATE fMarza := " "
      @ form_x_koord() + 10, form_y_koord() + 2    SAY "Magacin. Marza            :" GET _TMarza VALID _Tmarza $ "%AU" PICTURE "@!"
      @ form_x_koord() + 10, form_y_koord() + 40 GET _Marza PICTURE PicDEM
      @ form_x_koord() + 10, Col() + 1 GET fMarza PICT "@!"
      IF koncij->naz == "P2"
         @ form_x_koord() + 12, form_y_koord() + 2    SAY "PLANSKA CIJENA (PLC)        :"
      ELSE
         @ form_x_koord() + 12, form_y_koord() + 2    SAY "VELEPRODAJNA CJENA  (VPC)   :"
      ENDIF
      @ form_x_koord() + 12, form_y_koord() + 50 GET _VPC    PICTURE PicDEM;
