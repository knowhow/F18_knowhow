...
IF cVarijanta!="3"
    ...
    cMjesec := gMjesec
    cGodina := gGodina
    cPrimR1 := cPrimR2 := cPrimR3 := SPACE(60)
    cObracun := gObracun
    cPrimR1 := PADR( "01;", 60 )
...
IF cVarijanta!="3"
    ...
    Box(,2,30)
        ...
        ENDIF
        cFilt := IF( EMPTY(cIdRj), ".t.", "IDRJ==" + _filter_quote(cIdRj)) + ".and." + ;
...
FOR i:=1 TO 100
    IF FIELDPOS("I"+RIGHT("00"+ALLTRIM(STR(i)),2))==0
        nPoljaPr := i-1
        EXIT
    ENDIF
    nPoljaPr:=i
...
IF cVarijanta!="3"
    ...
    if empty(cidrj)
        ...
        ? Lokal("RJ:"), cIdrj + " - " + Ocitaj( F_LD_RJ, cIdRj, "naz" )
    endif
