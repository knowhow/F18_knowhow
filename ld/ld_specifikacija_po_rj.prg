...
FUNCTION ld_specifikacija_po_rj()
   ...
   DO WHILE .T.
      ...
      @ get_x_koord() + 2, get_y_koord() + 2   SAY "Radne jedinice (prazno-sve): "   GET qqRj PICT "@S20"
      @ get_x_koord() + 3, get_y_koord() + 2   SAY "Mjesec od: "                     GET cMjesecOd PICT "99"
      @ get_x_koord() + 3, Col() + 2 SAY "do"                              GET cMjesecDo PICT "99"
      @ get_x_koord() + 4, get_y_koord() + 2   SAY "Godina: "                        GET nGodina   PICT "9999"
      IF ld_vise_obracuna()
         @ get_x_koord() + 4, Col() + 2 SAY8 "Obračun:" GET cObracun WHEN HelpObr( .T., cObracun ) VALID ValObr( .T., cObracun )
      ENDIF
      @ get_x_koord() + 5, get_y_koord() + 2   SAY8 "Šifre primanja (prazno-sve):"   GET qqPrimanja PICT "@S30"
      READ
      ESC_BCR
      BoxC()
