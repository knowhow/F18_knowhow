...
do while !EOF()
	...
	do while !EOF() .and. field->_oid_ == nOid
		...
		if (nCnt > 1)
			if field->_oid_ == nOid
				// uporedi zapise
				if pos_doks->(idpos + brdok + DToS(datum) + idvd) == cFirma + cBrDok + DToS(dDatum) + cIdVd
					// dupli oid isti zapisi
				
					// dodaj u pom.tabelu sta si izbrisao
					AddToErrors("W", "OIDDEL", pos_doks->idvd + "-" + pos_doks->brdok + " od " + DToC(pos_doks->datum), "DOKS.DBF: pobrisan OID " + ALLTRIM(STR(field->_oid_)) + "!")
					// brisi ga
					select pos_doks
					delete
				else
					// dupli oid razliciti zapisi
					AddToErrors("C", "OIDERR", pos_doks->idvd + "-" + pos_doks->brdok + " od " + DToC(pos_doks->datum), "DOKS.DBF: Postoji dupli OID, podaci polja nisu identicni!")
					select pos_doks
				endif
			endif
		endif
