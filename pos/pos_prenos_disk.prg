...
AADD(opcexe,{|| pos_prenos_diskete()})
AADD(opc,"2. prijem dokumenata   <= ")
AADD(opcexe,{|| pos_povrat_diskete()})
AADD(opc,"3. podesavanje prenosa i prijema")
AADD(opcexe,{|| pos_prenos_setup()})
Menu_SC("posd")
closeret
return
...
/*! \fn pos_prenos_diskete()
 *  \brief Prenos na diskete
 */
 
function pos_prenos_diskete()
*{
local nRec
...
/*! \fn pos_povrat_diskete()
 *  \brief Povrat podataka 
 */
function pos_povrat_diskete()
*{
local nRec
local cDiff:=""
private cLokPrenosa:="A:\"
...
function pos_prenos_setup(lIni)
local GetList:={}
