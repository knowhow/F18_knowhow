...
select pos_doks; set order to 1
// "1", "IdPos+IdVd+dtos(datum)+BrDok"
set scope to
seek gIdPos+"42"+dtos(dDat)  //postavi scope
do while !eof() .and. gIdPos+"42"+dtos(dDat)==pos_doks->(idpos+idvd+dtos(datum))
     
     if  !(Vrijeme>=cVrijOd .and. Vrijeme<=cVrijDo)
       skip; loop
     endif

     cBrojR:=pos_doks->BrDok; cIdPos:=pos_doks->IdPos; cDatum:=dtos(pos_doks->datum)
...
if cNajstariji != "-" .and. (!IsPlanika()) .and. Pitanje(,"Izvrsiti sortiranje racuna ?","N")=="D"
	...
	do while !EOF() .and. pos_doks->IdPos == _IdPos .and. pos_doks->IdVd == VD_RN .and. pos_doks->IdPos < "X"

		++ nSkCnt
		@ m_x+3,m_y+2 SAY "Sklanjam rn.br: 42-" + ALLTRIM(pos_doks->brdok)
